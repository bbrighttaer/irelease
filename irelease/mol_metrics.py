...
# with minor refactoring and additions
...
def readNPModel(filename='NP_score.pkl.gz'):
    ...
    if filename == 'NP_score.pkl.gz':
        filename = os.path.join(os.path.dirname(irelease.__file__), filename)
...
def compute_results(samples, results={}, verbose=True):
...
def batch_external_diversity(smiles, set_smiles):
    rand_mols = [Chem.MolFromSmiles(s) for s in smiles]
...
def batch_hardnovelty(smiles, train_smiles):
    vals = [hard_novelty(smile, train_smiles) if verify_sequence(smile) else 0 for smile in smiles]
    return vals
...
def batch_druglikeliness(smiles, train_smiles):
    vals = [druglikeliness(s, train_smiles) if verify_sequence(s) else 0 for s in smiles]
    return vals
...
def metrics_loading():
    ...
    loadings['external_diversity'] = lambda *args: None
