...
class GuidedRewardLearningIRL(DRLAlgorithm):
    ...
    def __init__(self, model, optimizer, demo_gen_data, agent_net, agent_net_init_func, agent_net_init_func_args, k=10,
        ...
        self.bc_data = BinaryClassificationData(buffer_size, device=self.device)
    ...
    def fit(self, trajectories):
        ...
        new_smiles, valid_vec_samp = canonical_smiles(d_traj, has_start_end_tokens=True)
        for i, v in enumerate(valid_vec_samp):
            if v == 1:
                valid_smiles.append(new_smiles[i])
            else:
                invalid_smiles.append(d_traj[i])
        ...
        losses = []
        bce_losses = []
        self.bc_data.populate_pos(set(valid_smiles))
        self.bc_data.populate_neg(set(invalid_smiles))
