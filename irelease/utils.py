...
def canonical_smiles(smiles, sanitize=True, throw_warning=False, has_start_end_tokens=False):
    """
    ...
    When sanitize is True the function is analogous to:
        ...
        :param has_start_end_tokens: if true then '<' and '>' (start and end tokens) are present and should be
        removed.
    """
    new_smiles = []
    valid_vec = []
...
class ReplayBuffer:
    ...
    def sample(self, batch_size, replace=True):
        if len(self.buffer) <= batch_size:
            return self.buffer
        keys = np.random.choice(len(self.buffer), batch_size, replace=replace)
