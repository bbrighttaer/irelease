"""
Source: https://github.com/isayev/ReLeaSE
...
from gpmt.utils import time_since
...
class StackAugmentedRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, layer_type='GRU',
                 ...
                 stack_width=None, stack_depth=None, use_cuda=False,
        ...
        if layer_type not in ['GRU', 'LSTM']:
            raise Exception('Layer type must be GRU or LSTM')
        ...
        if self.has_stack:
            self.stack_controls_layer = nn.Linear(in_features=self.hidden_size * self.num_dir, out_features=3)
    ...
    def load_model(self, path, loc='cpu'):
        """
        ...
        """
        weights = torch.load(path, map_location=torch.device(loc))
    ...
    def train_step(self, inp, target):
        ...
        return loss.item() / len(inp), outputs
